...
Often, the response is a JSON document. You can process it further with [JSON-processing SQL functions](https://docs.dune.com/query-engine/Functions-and-operators/json). Other response formats, like html, are also supported.
...
**Response size limit:** Maximum accepted response body size is 2\_097\_152 bytes (2 MiB).

**Request size limit:** Maximum accepted request body size for `http_post()` is 1\_000\_000 bytes.
...
### JSON processing with http\_get()
...
### Using http\_get() in a subquery as a static filter
